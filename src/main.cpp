...
int main() {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			// The data format for each car is: [ { id, x, y, vx, vy, s, d }, {...} ]
          	auto others_cars = j[1]["sensor_fusion"];
			...
			for (int i = 0; i < others_cars.size(); i++) {
				// remember: The data format for each car is: [ { id, x, y, vx, vy, s, d }, {...} ]
				// Car is in my lane
				float d = others_cars[i][6];
				...
				double vx = others_cars[i][3];
				double vy = others_cars[i][4];
				double other_car_speed = sqrt(vx*vx + vy*vy);
				double other_car_s = others_cars[i][5];
			...
			double last_pt_x = car_x;
			double last_pt_y = car_y;
			double last_pt_yaw = deg2rad(car_yaw);
			...
			if (prev_size < 2) {
				...
				// Last point
				last_pt_x = previous_path_x[prev_size-1];
				last_pt_y = previous_path_y[prev_size-1];

				// 2nd-to-last point
				double last_pt_x2 = previous_path_x[prev_size-2];
				double last_pt_y2 = previous_path_y[prev_size-2];
				last_pt_yaw = atan2(last_pt_y-last_pt_y2, last_pt_x-last_pt_x2);

				// Use two points that make the path tangent to the path's previous endpoint
				ptsx.push_back(last_pt_x2);
				ptsx.push_back(last_pt_x);

				ptsy.push_back(last_pt_y2);
				ptsy.push_back(last_pt_y);
			}
			...
			for (int i = 0; i < ptsx.size(); i++) {
				...
				double shift_x = ptsx[i] - last_pt_x;
				double shift_y = ptsy[i] - last_pt_y;

				ptsx[i] = (shift_x * cos(0-last_pt_yaw) - shift_y * sin(0-last_pt_yaw));
				ptsy[i] = (shift_x * sin(0-last_pt_yaw) + shift_y * cos(0-last_pt_yaw));
			}
			...
			for (int i = 1; i <= 50 - previous_path_x.size(); i++) {
				...
				x_point = (x_ref * cos(last_pt_yaw) - y_ref*sin(last_pt_yaw));
				y_point = (x_ref * sin(last_pt_yaw) + y_ref*cos(last_pt_yaw));
  ...
  h.onDisconnection([&h](uWS::WebSocket<uWS::SERVER> ws, int code,
    ...
    ws.close();
    std::cout << "Discode.connected" << std::endl;
  });
